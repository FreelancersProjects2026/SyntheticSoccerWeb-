      ...
      >
        ...
        <div className="flex h-14 shrink-0 items-center border-b border-[#F0EFED] px-[18px]">
          ...
          >
            ...
            <span className="text-[12px] font-bold tracking-[0.05em] whitespace-nowrap text-[#072f1a]">
              CANCHA
            </span>
          </div>
          <button
        ...
        {/* Nav sections */}
        <nav className="flex-1 overflow-x-hidden overflow-y-auto px-[10px] py-3">
          {NAV_SECTIONS.map((section, si) => (
            <div key={si} className={si > 0 ? 'mt-2' : ''}>
              <p
                className={`px-[10px] pt-1.5 pb-1 text-[9px] font-semibold tracking-[0.14em] text-[#BCBAB5] uppercase transition-opacity duration-200 ${
                  collapsed ? 'md:opacity-0' : 'opacity-100'
                }`}
              >
                {section.label}
                  ...
                  <div key={item.path} className="group/item relative">
                    ...
                    >
                      {({ isActive }) => (
                        <>
                          <span
                            className={`shrink-0 ${isActive ? 'text-[#12D176]' : 'text-current'}`}
                          >
                            {item.icon}
                          </span>
                          <span
